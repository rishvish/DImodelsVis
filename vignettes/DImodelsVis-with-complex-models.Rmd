...
The only difference here is that in addition to the model object we also need to specify the `prop` parameter describing the name of the compositional variables in the model.
...
A benefit of using the data preparation functions is that, it's also possible to create the data using model coefficients. This is particularly useful for cases when your statistical model is fit using a different framework (SAS, SPSS, etc.) and it's difficult to fit the same model in R. We could simply import the coefficients from the external model and use those to create our visualisations.
```{r}
...
# Note when using model-coefficients, the data should be in the same order as the model coefficients
...
# Use coefficients and vcov parameter to specify the coefficients and variance-covariance matrix respectively.
# Note: specify a vcov matrix is necessary only if the user want the uncertainty associated with the prediction
...
We fix `n-3` variables to have a constant value $x_1, x_2, x_3, ..., x_{n-3}$ such that $x = x_1 + x_2 + x_3 + ... x_{n - 3}$ and $0 \leq x \leq 1$ and vary the proportion of the remaining three variables between $0$ and $1-x$ to visualise the change in the predicted response as a contour map within a ternary diagram.
